...
def upsert_user(tg_user: dict) -> dict:
    ...
    if row is None:
        ...
        c.commit()
        row = c.execute("SELECT * FROM users WHERE telegram_id=?", (tg_user["id"],)).fetchone()
    # root из ADMIN_IDS всегда админ мини-аппа — без отдельной выдачи /admin
    if tg_user["id"] in config.ADMIN_IDS and not row["is_admin"]:
        c.execute("UPDATE users SET is_admin=1 WHERE telegram_id=?", (tg_user["id"],))
        c.commit()
        row = c.execute("SELECT * FROM users WHERE telegram_id=?", (tg_user["id"],)).fetchone()
    c.close()
    return dict(row)
