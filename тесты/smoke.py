...
Запуск: venv/bin/python тесты/smoke.py   (поднимает свой сервер на порту 9543)
"""
import hashlib
import hmac
import json
...
sys.path.insert(0, str(ROOT / "бот"))
...
from миниапп.helpers import validate_init_data  # noqa: E402
...
server = subprocess.Popen(
    [str(ROOT / "venv" / "bin" / "python"), "-m", "миниапп.server"],
...
fixture = (ROOT / "тесты" / "cp_fixture.json").read_text(encoding="utf-8")
...
from эталоны import ЭТАЛОНЫ, ПАЧКИ  # noqa: E402

import ocr  # noqa: E402

n = ocr.normalize_result(dict(ЭТАЛОНЫ["photo_2_2026-09-24_19-49-40.jpg"]))
step("OCR: кейс 120:00+пенальти обязателен",
     (n["score_home"], n["score_away"]) == (3, 3) and n["penalties"] == {"home": 5, "away": 4})
pack = ПАЧКИ[1]
merged = ocr.merge_results([ocr.normalize_result(dict(ЭТАЛОНЫ[f])) for f in pack["files"]])
