...
Root-команды: /промокод, /админ (toggle users.is_admin), /судья и /клуб — в блоках 4.
...
async def cmd_promo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/промокод <КОД> <сумма> [активаций=0(∞)] [YYYY-MM-DD]"""
...
async def cmd_app_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/админ <@user|id> — выдать/снять users.is_admin (toggle)."""
    if not core.is_root(update.effective_user.id):
        await update.message.reply_text("⛔ Только root.")
        return
    if not context.args:
        await update.message.reply_text("Формат: /админ <@user|id>")
        return
...
HANDLERS = [
    ("command", "промокод", cmd_promo),
    ("command", "админ", cmd_app_admin),
]


# ===== /финал — итоги сезона (решение 09: применяет админ, не фоновый job) =====

async def cmd_season_final(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/финал <турнир_id> → превью; подтверждение кнопкой."""
    import league
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Формат: /финал <турнир_id> — сначала покажу превью.")
        return
...
async def cb_season_final_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines.append("Elo нового сезона стартует с 1000 (создай: /сезон).")
    await update.callback_query.edit_message_text("\n".join(lines))
    core.audit(int(tid), update.effective_user.id, "season_finalized", "bot")


HANDLERS = HANDLERS + [
    ("command", "финал", cmd_season_final),
